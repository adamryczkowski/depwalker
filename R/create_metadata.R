...
append_extra_code<-function(metadata, filepath, flag.checksum, flag.binary, flag.r)
{
  ...
  if(flag.binary) {
    filedigest<-as.character(tools::md5sum(filepath))
  } else {
    src<-readLines(depwalker:::get.fullpath(metadata, filepath))
    if(flag.r) {
      src2<-normalize_code_string(src)
      flag_rewrite<-FALSE
      if(length(src)!=length(src2)){
        flag_rewrite<-TRUE
      } else {
        flag_rewrite<-!all(src==src2)
      }
      src<-src2
      if(flag_rewrite) {
        writeLines(src,depwalker:::get.fullpath(metadata, filepath) )
      }
    }
    filedigest<-calculate_one_digest(src)
  }
  extrasources[filepath]<-list(list(path=filepath, flag.checksum=flag.checksum, flag.binary=flag.binary,
                                    flag.r=flag.r, digest=filedigest))
