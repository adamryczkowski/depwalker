...
test_that("Test getting task from R memory (1)", {
  ...
  env<-new.env()
  depwalker:::load.object(metadata=m, metadata.path=file.path(tmpdir, "task14"), target.environment = env)
  ...
  depwalker:::load.object(metadata=m, metadata.path=file.path(tmpdir, "task14"), objectname="x", target.environment = env)
